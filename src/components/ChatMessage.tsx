...
const ChatMessage = ({ message, isTyping = false }: ChatMessageProps) => {
    ...
    if (message.role === 'assistant' && !isTyping) {
      ...
      const typeWriter = () => {
        if (index < text.length) {
          ...
          index++;
          setTimeout(typeWriter, 10); // Faster typing for better UX
        } else {
          setShowCursor(false);
        }
      };
      
      setTimeout(typeWriter, 50); // Faster start
    } else {
  ...
  return (
    <div className={`flex gap-2 sm:gap-3 md:gap-4 p-2 sm:p-3 md:p-6 animate-fade-in ${isUser ? 'justify-end' : 'justify-start'}`}>
      {!isUser && (
        <div className="flex-shrink-0">
          <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-gradient-primary flex items-center justify-center shadow-glow">
            <PandaLogo className="w-3 h-3 sm:w-4 sm:h-4 md:w-6 md:h-6" animate={isTyping} />
          </div>
        </div>
      )}
      ...
      <div className={`max-w-[85%] sm:max-w-[80%] md:max-w-[75%] ${isUser ? 'order-first' : ''}`}>
        <div className={`
          ...
          p-2 sm:p-3 md:p-4 transition-all duration-300 hover:shadow-glow
        `}>
          {/* User uploaded image */}
          {message.image && (
            <div className="mb-2 sm:mb-3 md:mb-4 group">
              <img 
                src={message.image} 
                alt="User uploaded" 
                className="max-w-full max-h-48 sm:max-h-64 md:max-h-80 rounded-lg transition-transform duration-300 hover:scale-105 cursor-pointer object-cover" 
              />
            </div>
          )}
          ...
          {isCodeBlock ? (
            <pre className={`
              font-mono text-xs sm:text-sm overflow-x-auto p-2 sm:p-3 md:p-4 rounded-lg border max-h-64 sm:max-h-80
            ...
            <div className="prose prose-invert max-w-none">
              <div className="whitespace-pre-wrap leading-relaxed text-xs sm:text-sm md:text-base">
      ...
      {isUser && (
        <div className="flex-shrink-0">
          <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-secondary flex items-center justify-center border border-border">
